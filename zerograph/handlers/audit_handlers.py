"""
Taint Analysis MCP Tools for ZeroGraph Server
...
from zerograph.lib.query_rendering import escape_scala_string
from zerograph.lib.validators import validate_codebase_hash
from zerograph.templates.loader import QueryLoader
...
def _build_file_filter_regex(filename: str) -> str:
    """Build a graph-engine-compatible regex for path-boundary anchored file filtering.
...
def _build_graph_name_pattern(patterns: list) -> str:
    """Build a graph .name() regex pattern from a list of function names.

    Handles qualified names (e.g., 'os.system' -> 'system') since native
...
def _find_taint_flows_auto(
    ...
    # Build graph regex patterns
    source_regex = _build_graph_name_pattern(src_patterns)
    sink_regex = _build_graph_name_pattern(snk_patterns)
    sanitizer_regex = _build_graph_name_pattern(san_patterns) if san_patterns else ""
...
def register_audit_handlers(mcp, services: dict):
...
Args:
    codebase_hash: The codebase hash from zg_index_repo.
...
Notes:
    ...
    - Use node_id from results with zg_trace_flows.

Examples:
    zg_trace_inputs(codebase_hash="abc", language="c")
    zg_trace_inputs(codebase_hash="abc", source_patterns=["read_from_socket"])""",
    )
    def zg_trace_inputs(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        ...
        try:
            ...
            if not codebase_info or not codebase_info.cpg_path:
                raise ValidationError(f"CPG not found for codebase {codebase_hash}. Generate it first using zg_index_repo.")
            ...
            # Build graph .name() regex from patterns, extracting short names
            # from qualified patterns (e.g., 'os.system' -> 'system')
            joined = _build_graph_name_pattern(patterns)
            ...
            return _cached_taint_query(services, "zg_trace_inputs", codebase_hash, cache_params, _execute)
...
Examples:
    zg_trace_sinks(codebase_hash="abc", language="c")
    zg_trace_sinks(codebase_hash="abc", sink_patterns=["custom_exec"])""",
    )
    def zg_trace_sinks(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_trace_sinks", codebase_hash, cache_params, _execute)
    ...
    @mcp.tool(
        description="""Find taint flows from a source to a sink using native native dataflow analysis.

Detects data flow from a specific source node to a specific sink node.
Uses native reachableByFlows() for accurate taint tracking including pointer aliasing,
array propagation, and struct fields.

Supports two modes:

MODE 1 - Manual (default): Provide a specific source and sink.
  - Use `zg_trace_inputs` first to get source locations/IDs.
  - Use `zg_trace_sinks` first to get sink locations/IDs.
...
Args:
    ...
    source_node_id: (Manual mode) Alternative: node ID from zg_trace_inputs output.
    sink_node_id: (Manual mode) Alternative: node ID from zg_trace_sinks output.
...
Examples:
    ...
    zg_trace_flows(codebase_hash="...", mode="auto")
    zg_trace_flows(codebase_hash="...", mode="auto", language="c", filename="main.c")
    zg_trace_flows(codebase_hash="...", mode="auto", source_patterns=["getenv"], sink_patterns=["system", "strcpy"])

    # Manual mode — specific source and sink
    zg_trace_flows(codebase_hash="...", source_location="main.c:42", sink_location="utils.c:100")
    zg_trace_flows(codebase_hash="...", source_node_id=12345, sink_node_id=67890)""",
    )
    def zg_trace_flows(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        ...
        source_node_id: Annotated[Optional[int], Field(description="(Manual mode) Node ID from zg_trace_inputs output")] = None,
        sink_node_id: Annotated[Optional[int], Field(description="(Manual mode) Node ID from zg_trace_sinks output")] = None,
        ...
        """Find data flow paths between source and sink using native native taint analysis."""
        try:
            ...
            if legacy_args:
                raise ValidationError(
                    ...
                    "Use 'zg_trace_inputs' to find sources by pattern, then use the resulting 'node_id' here. "
            ...
            if not has_sink_loc and not has_sink_id:
                raise ValidationError(
                    ...
                    f"CORRECT USAGE WORKFLOW:\n"
                    f"Option A — Manual mode:\n"
                    f"  1. Call `zg_trace_sinks(...)` first to find valid sinks.\n"
                    f"  2. Pick a sink, note its `node_id` or `filename:line`.\n"
                    f"  3. Call `zg_trace_flows` again providing that sink.\n\n"
                    f"Option B — Auto mode (recommended for audits):\n"
                    f"  zg_trace_flows(codebase_hash='...', mode='auto')\n\n"
                    f"EXAMPLE:\n"
                    f"zg_trace_flows(\n"
            ...
            return _cached_taint_query(services, "zg_trace_flows", codebase_hash, cache_params, _execute)
...
Examples:
    zg_slice_callsite(codebase_hash="abc", location="main.c:42")
    zg_slice_callsite(codebase_hash="abc", location="parser.c:500:memcpy", direction="backward", max_depth=3)
    zg_slice_callsite(codebase_hash="abc", location="module/file.c:100", direction="forward")""",
    )
    def zg_slice_callsite(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_slice_callsite", codebase_hash, cache_params, _execute)
...
Examples:
    zg_slice_binding(codebase_hash="abc", location="main.c:50", variable="len", direction="backward")""",
    )
    def zg_slice_binding(
        codebase_hash: str,
        location: str,
        variable: str,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_slice_binding", codebase_hash, cache_params, _execute)
...
3. **Deep Interprocedural**: Uses native reachableByFlows() to track freed pointers
...
Notes:
    ...
    - Use zg_slice_callsite to understand control flow around specific locations.
    - Use zg_trace_flows for alternative dataflow analysis approach.""",
    )
    def zg_scan_uaf(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_uaf", codebase_hash, cache_params, _execute)
...
Returns:
    ...
    def zg_scan_double_free(
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_double_free", codebase_hash, cache_params, _execute)
...
4. **Deep Interprocedural**: Uses native reachableByFlows() to track allocated pointers
...
Notes:
    - Includes deep interprocedural analysis using native dataflow engine.
    - Use zg_slice_callsite for deeper control-flow context around specific locations.
    - Use zg_trace_flows to check if allocation arguments come from external input.""",
    )
    def zg_scan_null_deref(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        limit: Annotated[int, Field(description="Maximum results to return")] = 100,
        timeout: Annotated[int, Field(description="Query timeout in seconds")] = 300,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_null_deref", codebase_hash, cache_params, _execute)
...
4. **Deep interprocedural**: Uses native reachableByFlows() to track arithmetic results
...
Notes:
    - Includes deep interprocedural analysis using native dataflow engine.
    - Use zg_slice_callsite for deeper control-flow context around specific locations.
    - Use zg_trace_flows to check if arithmetic operands come from external input.""",
    )
    def zg_scan_int_overflow(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        limit: Annotated[int, Field(description="Maximum results to return")] = 100,
        timeout: Annotated[int, Field(description="Query timeout in seconds")] = 300,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_int_overflow", codebase_hash, cache_params, _execute)
...
Examples:
    zg_scan_fmt_string(codebase_hash="abc")
    zg_scan_fmt_string(codebase_hash="abc", filename="log.c")""",
    )
    def zg_scan_fmt_string(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        limit: Annotated[int, Field(description="Maximum results to return")] = 100,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_fmt_string", codebase_hash, cache_params, _execute)
...
Examples:
    zg_scan_heap_oob(codebase_hash="abc")
    zg_scan_heap_oob(codebase_hash="abc", filename="buffer.c")""",
    )
    def zg_scan_heap_oob(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        limit: Annotated[int, Field(description="Maximum results to return")] = 100,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_heap_oob", codebase_hash, cache_params, _execute)
...
Examples:
    zg_scan_stack_oob(codebase_hash="abc")
    zg_scan_stack_oob(codebase_hash="abc", filename="parser.c")""",
    )
    def zg_scan_stack_oob(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        limit: Annotated[int, Field(description="Maximum results to return")] = 100,
        timeout: Annotated[int, Field(description="Query timeout in seconds")] = 240,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_stack_oob", codebase_hash, cache_params, _execute)
...
Examples:
    zg_scan_toctou(codebase_hash="abc")
    zg_scan_toctou(codebase_hash="abc", filename="fs_utils.c")""",
    )
    def zg_scan_toctou(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_toctou", codebase_hash, cache_params, _execute)
...
Examples:
    zg_scan_uninit(codebase_hash="abc")
    zg_scan_uninit(codebase_hash="abc", filename="parser.c")""",
    )
    def zg_scan_uninit(
        codebase_hash: Annotated[str, Field(description="The codebase hash from zg_index_repo")],
        filename: Annotated[Optional[str], Field(description="Optional filename regex to filter results")] = None,
        ...
        try:
            ...
            return _cached_taint_query(services, "zg_scan_uninit", codebase_hash, cache_params, _execute)
