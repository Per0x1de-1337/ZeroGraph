"""
MCP Prompt Templates for ZeroGraph Server

Pre-built security analysis workflows that guide LLMs through
ZeroGraph's tools for systematic vulnerability discovery and
...
ZeroGraph's detectors assign a **Confidence** level (HIGH / MEDIUM) to each finding based on:
        ...
        if focus_area and focus_area != "all":
            focus_map = {
                ...
                "memory": (
                    ...
                    "missing bounds checks, and TOCTOU race conditions. Run zg_scan_uaf, "
                    "zg_scan_double_free, zg_scan_bounds_guard, zg_scan_null_deref, "
                    "zg_scan_int_overflow, and zg_scan_toctou."
                ),
                "authentication": (
...
7. `zg_scan_uaf(codebase_hash="{hash}")` — detect use-after-free vulnerabilities
8. `zg_scan_double_free(codebase_hash="{hash}")` — detect double-free vulnerabilities
9. `zg_scan_bounds_guard(codebase_hash="{hash}")` — find buffer accesses missing bounds checks
10. `zg_scan_null_deref(codebase_hash="{hash}")` — detect null pointer dereference vulnerabilities (CWE-476)
11. `zg_scan_int_overflow(codebase_hash="{hash}")` — detect integer overflow/underflow before allocation or array indexing (CWE-190)
12. `zg_scan_toctou(codebase_hash="{hash}")` — detect TOCTOU race conditions (CWE-367) where access()/stat() is followed by open() on the same path""".format(hash=codebase_hash)
...
1. Call `zg_project_stats(codebase_hash="{codebase_hash}")` to understand scope (language, files, methods, calls)
2. Call `list_source_files(codebase_hash="{codebase_hash}")` to understand the project structure
3. Call `zg_audit_git_history(codebase_hash="{codebase_hash}")` to find historical vulnerability hints from git history

## Phase 2: Attack Surface Mapping
4. Call `zg_trace_inputs(codebase_hash="{codebase_hash}"{f', language="{language}"' if language else ''})` to enumerate all external input points
5. Call `zg_trace_sinks(codebase_hash="{codebase_hash}"{f', language="{language}"' if language else ''})` to enumerate all dangerous operations

## Phase 3: Automated Taint Analysis
6. Call `zg_trace_flows(codebase_hash="{codebase_hash}", mode="auto"{f', language="{language}"' if language else ''})` to discover all confirmed source-to-sink data flows
{memory_instructions}
...
- Call `zg_slice_callsite(codebase_hash="{codebase_hash}", ...)` to trace data origins backward
- Call `zg_slice_binding(codebase_hash="{codebase_hash}", ...)` for specific variable tracking
...
Call `zg_project_stats(codebase_hash="{codebase_hash}")` to confirm this is a C or C++ codebase.
...
Call `zg_scan_uaf(codebase_hash="{codebase_hash}"{file_filter})` to find cases where memory is accessed after being freed.
- CWE-416: Use After Free
- For each finding, note the free() call location and the subsequent use location

## Step 3: Double-Free Detection
Call `zg_scan_double_free(codebase_hash="{codebase_hash}"{file_filter})` to find cases where the same pointer is freed twice.
- CWE-415: Double Free
- For each finding, note both free() locations and the pointer involved

## Step 4: Buffer Overflow Detection
Call `zg_trace_sinks(codebase_hash="{codebase_hash}", language="c", sink_patterns=["memcpy", "strcpy", "strcat", "sprintf", "gets", "memmove", "strncpy", "strncat", "vsprintf", "sscanf"])` to find buffer-writing operations.
For each dangerous buffer operation found, call `zg_scan_bounds_guard(codebase_hash="{codebase_hash}", buffer_access_location="<file>:<line>")` to check if bounds validation exists nearby.
- CWE-120: Buffer Copy without Checking Size
- CWE-787: Out-of-bounds Write

## Step 5: Null Pointer Dereference Detection
Call `zg_scan_null_deref(codebase_hash="{codebase_hash}"{file_filter})` to find cases where pointers from allocation functions (malloc, calloc, realloc, fopen, strdup, mmap, etc.) are dereferenced without NULL checks.
- CWE-476: NULL Pointer Dereference
- For each finding, note the allocation site and the unchecked dereference location

## Step 6: Integer Overflow Detection
Call `zg_scan_int_overflow(codebase_hash="{codebase_hash}"{file_filter})` to find arithmetic operations (multiplication, left-shift, addition, subtraction) that could overflow before being used as allocation sizes or array indices.
- CWE-190: Integer Overflow or Wraparound
- For each finding, note the arithmetic expression, operation type, and risk level

## Step 7: TOCTOU Detection
Call `zg_scan_toctou(codebase_hash="{codebase_hash}"{file_filter})` to find cases where a file is checked with access()/stat()/lstat() and then opened or acted on in a separate step.
...
For each confirmed vulnerability, call `zg_slice_callsite(codebase_hash="{codebase_hash}", ...)` with direction="backward" to trace where the data or pointer originated.
...
Call `zg_file_range(codebase_hash="{codebase_hash}", filename="{source_location.split(':')[0]}", start_line={max(1, int(source_location.split(':')[1]) - 10)}, end_line={int(source_location.split(':')[1]) + 10})` to see the code around the source.

## Step 2: Examine Sink Context
Call `zg_file_range(codebase_hash="{codebase_hash}", filename="{sink_location.split(':')[0]}", start_line={max(1, int(sink_location.split(':')[1]) - 10)}, end_line={int(sink_location.split(':')[1]) + 10})` to see the code around the sink.

## Step 3: Check for Taint Flow
Call `zg_trace_flows(codebase_hash="{codebase_hash}", source_location="{source_location}", sink_location="{sink_location}")` to confirm whether a data flow path exists.

## Step 4: Trace the Path
Call `zg_slice_callsite(codebase_hash="{codebase_hash}", location="{sink_location}", direction="backward")` to understand all data dependencies reaching the sink.
Call `zg_slice_binding(codebase_hash="{codebase_hash}", ...)` to trace specific variables along the path.

## Step 5: Analyze Control Flow
Call `zg_control_flow(codebase_hash="{codebase_hash}", ...)` for the function containing the sink to understand branching and conditions that must be met.
...
Call `zg_trace_flows(codebase_hash="{codebase_hash}", mode="auto"{lang_param})` to automatically discover all data flow paths from untrusted sources to dangerous sinks.
...
1. Call `zg_file_range(codebase_hash="{codebase_hash}", ...)` to see source and sink code in context
2. Call `zg_slice_callsite(codebase_hash="{codebase_hash}", ...)` with direction="backward" from the sink
3. Call `zg_slice_binding(codebase_hash="{codebase_hash}", ...)` to trace the specific tainted variable
4. Call `zg_control_flow(codebase_hash="{codebase_hash}", ...)` to understand conditions and branches
...
Call `zg_project_stats(codebase_hash="{codebase_hash}")` to understand the scale and language.

## Step 2: Enumerate Entry Points
Call `zg_trace_inputs(codebase_hash="{codebase_hash}"{lang_param})` to discover ALL external input points.
...
Call `zg_trace_sinks(codebase_hash="{codebase_hash}"{lang_param})` to discover ALL dangerous operations.
...
Call `zg_trace_flows(codebase_hash="{codebase_hash}", mode="auto"{lang_param})` to find all confirmed paths where untrusted data reaches dangerous operations.

## Step 5: Map Call Reachability
For each major entry point function (functions containing taint sources), call `zg_invocation_tree(codebase_hash="{codebase_hash}", method_name="<entry_function>", direction="outgoing", depth=3)` to map what code is reachable from each entry point.

## Step 6: Historical Context
Call `zg_audit_git_history(codebase_hash="{codebase_hash}")` to find any previously fixed vulnerabilities in git history.
...
Call `zg_symbol_params(codebase_hash="{codebase_hash}", method_name="{function_name}")` to understand all parameters and their types.

## Step 3: Map Call Relationships
Call `zg_invocation_tree(codebase_hash="{codebase_hash}", method_name="{function_name}", direction="incoming", depth=2)` to see who calls this function (attack entry points).
Call `zg_invocation_tree(codebase_hash="{codebase_hash}", method_name="{function_name}", direction="outgoing", depth=3)` to see what dangerous operations this function can reach.

## Step 4: Analyze Control Flow
Call `zg_control_flow(codebase_hash="{codebase_hash}", method_name="{function_name}")` to understand branching, loops, and error handling paths.

## Step 5: Check for Taint
Call `zg_trace_inputs(codebase_hash="{codebase_hash}")` and check if any sources exist within this function or its callers.
Call `zg_trace_sinks(codebase_hash="{codebase_hash}")` and check if any sinks exist within this function or its callees.
If both sources and sinks are found in the call chain, call `zg_trace_flows(codebase_hash="{codebase_hash}", mode="auto")` and filter for flows involving this function.

## Step 6: Trace Parameter Data Flow
For each parameter, call `zg_slice_binding(codebase_hash="{codebase_hash}", ...)` to understand where parameter values come from (backward) and where they flow to (forward).
...
- **Remediation**: Specific recommendations"""
            ...
            if line_number:
                ...
                snippet_call = f"""
Call `zg_file_range(codebase_hash="{codebase_hash}", filename="{filename}", start_line={start}, end_line={end})` to see the code around line {line_number}."""
            else:
                snippet_call = f"""
Call `zg_list_symbols(codebase_hash="{codebase_hash}", file_pattern="{filename}")` to enumerate all functions in this file."""
...
Call `zg_list_symbols(codebase_hash="{codebase_hash}", file_pattern="{filename}")` to see all methods defined in this file.

## Step 3: Find Input Points
Call `zg_trace_inputs(codebase_hash="{codebase_hash}")` and filter results for sources in `{filename}`.

## Step 4: Find Dangerous Operations
Call `zg_trace_sinks(codebase_hash="{codebase_hash}")` and filter results for sinks in `{filename}`.

## Step 5: Check Data Flows
Call `zg_trace_flows(codebase_hash="{codebase_hash}", mode="auto")` and filter for flows involving `{filename}`.

## Step 6: Memory Safety (if C/C++)
If the codebase is C/C++:
- Call `zg_scan_uaf(codebase_hash="{codebase_hash}", filename="{filename}")`
- Call `zg_scan_double_free(codebase_hash="{codebase_hash}", filename="{filename}")`
- Call `zg_scan_null_deref(codebase_hash="{codebase_hash}", filename="{filename}")`
- Call `zg_scan_int_overflow(codebase_hash="{codebase_hash}", filename="{filename}")`

## Step 7: Trace Vulnerabilities
For each vulnerability found{f" near line {line_number}" if line_number else ""}:
- Call `zg_slice_callsite(codebase_hash="{codebase_hash}", ...)` with direction="backward" to trace data origins
- Call `zg_invocation_tree(codebase_hash="{codebase_hash}", method_name="<function>", direction="incoming")` to understand who can trigger the vulnerable code
...
Call `zg_symbol_params(codebase_hash="{codebase_hash}", method_name="{function_name}")` to understand the function signature.

## Step 2: Understand Context
Call `zg_invocation_tree(codebase_hash="{codebase_hash}", method_name="{function_name}", direction="incoming", depth=2)` to see callers.
Call `zg_invocation_tree(codebase_hash="{codebase_hash}", method_name="{function_name}", direction="outgoing", depth=2)` to see callees.
Call `zg_control_flow(codebase_hash="{codebase_hash}", method_name="{function_name}")` to understand control flow."""
        elif filename:
            gather_steps = f"""## Step 1: Retrieve Code
Call `zg_list_symbols(codebase_hash="{codebase_hash}", file_pattern="{filename}")` to enumerate all functions in the file.
...
## Step 2: Understand Context
Call `zg_list_invocations(codebase_hash="{codebase_hash}", caller_pattern=".*", file_pattern="{filename}")` to see all calls made from this file.
For key functions, call `zg_control_flow(codebase_hash="{codebase_hash}", method_name="<name>")` to understand control flow."""
        else:
            gather_steps = f"""## Step 1: Codebase Overview
Call `zg_project_stats(codebase_hash="{codebase_hash}")` to understand size and language.
Call `list_source_files(codebase_hash="{codebase_hash}")` to see the project structure.
Call `zg_list_symbols(codebase_hash="{codebase_hash}")` to enumerate all functions.

## Step 2: Identify Critical Code
Focus the review on functions that:
- Handle external input (check `zg_trace_inputs(codebase_hash="{codebase_hash}")`)
- Perform dangerous operations (check `zg_trace_sinks(codebase_hash="{codebase_hash}")`)
- Are heavily called (use `zg_list_invocations(codebase_hash="{codebase_hash}")` to find high fan-in functions)
For each critical function, call `get_file_content(codebase_hash="{codebase_hash}", method_name="<name>")` and `zg_control_flow(codebase_hash="{codebase_hash}", method_name="<name>")` to review."""
...
- Call `zg_trace_inputs(codebase_hash="{codebase_hash}")` and `zg_trace_sinks(codebase_hash="{codebase_hash}")` to identify input/output points in scope.

### Injection Risks
- Call `zg_trace_flows(codebase_hash="{codebase_hash}", mode="auto")` and filter for flows in scope.
- For each confirmed flow, call `zg_slice_callsite(codebase_hash="{codebase_hash}", ...)` to verify there is no sanitization along the path.
