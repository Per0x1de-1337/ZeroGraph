"""Configuration management for the ZeroGraph Server."""
...
def load_config(config_path: Optional[str] = None) -> Config:
    ...
    if config_path and os.path.exists(config_path):
        ...
        return Config(
            ...
            engine=EngineConfig(
                binary_path=os.getenv("ENGINE_BINARY_PATH", defaults.ENGINE_BINARY_PATH),
                memory_limit=os.getenv("ENGINE_MEMORY_LIMIT", defaults.ENGINE_MEMORY_LIMIT),
                java_opts=os.getenv("ENGINE_JAVA_OPTS", defaults.ENGINE_JAVA_OPTS),
                server_host=os.getenv("ENGINE_SERVER_HOST", defaults.ENGINE_SERVER_HOST),
                server_port=int(os.getenv("ENGINE_SERVER_PORT", str(defaults.ENGINE_SERVER_PORT))),
                server_auth_username=os.getenv("ENGINE_SERVER_AUTH_USERNAME"),
                server_auth_password=os.getenv("ENGINE_SERVER_AUTH_PASSWORD"),
                port_min=int(os.getenv("ENGINE_PORT_MIN", str(defaults.ENGINE_PORT_MIN))),
                port_max=int(os.getenv("ENGINE_PORT_MAX", str(defaults.ENGINE_PORT_MAX))),
                server_init_sleep_time=float(os.getenv("ENGINE_SERVER_INIT_SLEEP_TIME", str(defaults.ENGINE_SERVER_INIT_SLEEP_TIME))),
                server_startup_timeout=int(os.getenv("ENGINE_SERVER_STARTUP_TIMEOUT", str(defaults.ENGINE_SERVER_STARTUP_TIMEOUT))),
                max_active_servers=int(os.getenv("MAX_ACTIVE_ENGINE_SERVERS", str(defaults.MAX_ACTIVE_ENGINE_SERVERS))),
            ),
            cpg=CPGConfig(
            ...
            telemetry=TelemetryConfig(
                ...
                service_name=os.getenv("OTEL_SERVICE_NAME", "zerograph"),
...
def _dict_to_config(data: dict) -> Config:
    ...
    return Config(
        ...
        engine=convert_config_section(
            EngineConfig, data.get("engine", data.get("joern", {}))
        ),
