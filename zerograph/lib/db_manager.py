...
class DBManager:
    """SQLite database manager for ZeroGraph"""

    def __init__(self, db_path: str = "zerograph.db"):
            ...
            with self._get_connection() as conn:
                ...
                conn.execute("""
                    INSERT OR REPLACE INTO codebases (
                        ...
                        cpg_path, engine_port, metadata, created_at, last_accessed
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    data["hash"],
