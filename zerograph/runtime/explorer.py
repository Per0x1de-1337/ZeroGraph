...
from zerograph.lib.validators import sanitize_path, validate_codebase_hash
...
class Explorer:
    ...
    def zg_list_symbols(
        ...
        full_result = self._get_cached_or_execute("zg_list_symbols", codebase_hash, cache_params, execute_query)
    ...
    def zg_list_files(
        ...
        # Always use the workspace snapshot copy — this is the exact state used for
        # CPG generation, and avoids relying on the original path still being accessible.
        workspace_snapshot = os.path.join(workspace_path, "repos", codebase_hash)
        if os.path.exists(workspace_snapshot) and os.path.isdir(workspace_snapshot):
            source_dir = workspace_snapshot
        elif codebase_info.source_type == "github":
            # Fallback: shouldn't normally be needed, but recalculate just in case
            from zerograph.handlers.index_handlers import get_cpg_cache_key
            cpg_cache_key = get_cpg_cache_key(
                codebase_info.source_type,
                codebase_info.source_path,
                codebase_info.language,
            )
            source_dir = os.path.join(workspace_path, "repos", cpg_cache_key)
        else:
    ...
    def zg_list_invocations(
        ...
        full_result = self._get_cached_or_execute("zg_list_invocations", codebase_hash, cache_params, execute_query)
    ...
    def zg_symbol_params(
        ...
        return self._get_cached_or_execute("zg_symbol_params", codebase_hash, cache_params, execute_query)
    ...
    def warm_up_cache(self, codebase_hash: str):
        ...
        tasks = [
            (self.zg_list_symbols, [codebase_hash]),
            (self.zg_list_files, [codebase_hash]),
            (self.zg_list_invocations, [codebase_hash]),
            (self.zg_symbol_params, [codebase_hash]),
