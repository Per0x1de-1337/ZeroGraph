"""
HTTP client for communicating with analysis engine API
"""
...
class EngineRpc:
    """Client for analysis engine HTTP API with connection pooling"""
    ...
    def __init__(self, host: str = "localhost", port: int = 8080, username: Optional[str] = None, password: Optional[str] = None, config: Optional[Dict[str, Any]] = None):
        """
        Initialize analysis engine client with connection pooling
    ...
    def check_health(self, timeout: int = 5) -> bool:
        """
        Quick health check to verify the analysis engine is responsive.
    ...
    def execute_query(
        ...
        try:
            ...
            return {
                "success": False,
                "stdout": "",
                "stderr": f"Query timeout after {timeout}s. The analysis engine may be overloaded. "
    ...
    def load_cpg(self, cpg_path: str, project_name: Optional[str] = None, timeout: int = 600) -> bool:
        """
        Load a CPG file into the analysis engine
        ...
        """
        try:
            # Use importCpg to load pre-built graph.bin file
            # Use workspace.resett to ensure clean state in the isolated workspace
            # We don't force project name to avoid potential API issues, letting the engine derive it from filename
